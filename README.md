...
<title>Anu Krupa — Marketplace (Demo)</title>
<meta name="description" content="Anu Krupa demo - Single-file marketplace (Admin/Seller/Buyer) with UPI simulated payments, videos, multi-lang" />
<style>
  :root{
    --primary: #0ea5a4;
    --accent: #7c3aed;
  ...
  *{box-sizing:border-box;font-family:Inter,system-ui,Segoe UI,Roboto,Arial;margin:0;padding:0}
  body{background:linear-gradient(135deg,var(--bg),#fff);color:#0b1220;min-height:100vh}
  header{background:linear-gradient(90deg,var(--primary),var(--accent));padding:12px 18px;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:12px}
  ...
  .hero{display:flex;gap:16px;align-items:center;padding:16px;border-radius:10px;background:linear-gradient(180deg,#fff,rgba(255,255,255,0.98));box-shadow:0 8px 24px rgba(2,6,23,0.06)}
  h1{font-size:20px}
  ...
  main{flex:1}
  aside{width:360px}
  .card{background:var(--card);padding:12px;border-radius:10px;box-shadow:0 8px 20px rgba(2,6,23,0.04)}
  .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
  .product-img{height:160px;border-radius:8px;background:#f6f8fb;display:flex;align-items:center;justify-content:center;color:var(--muted);overflow:hidden}
  .product-img img{width:100%;height:100%;object-fit:cover;display:block}
  .muted{color:var(--muted);font-size:13px}
  footer{margin-top:20px;padding:12px;text-align:center;color:var(--muted)}
  .modal-back{position:fixed;inset:0;background:rgba(2,6,23,0.5);display:flex;align-items:center;justify-content:center;z-index:120}
  .modal{background:#fff;padding:16px;border-radius:10px;width:920px;max-width:calc(100% - 30px);max-height:90vh;overflow:auto}
  label{display:block;margin-top:8px;font-size:13px;color:var(--muted)}
  input,select,textarea{width:100%;padding:8px;border-radius:8px;border:1px solid #eef6fb;margin-top:6px}
  table{width:100%;border-collapse:collapse}
  th,td{padding:8px;border-bottom:1px solid #f2f6fb;text-align:left}
  .small{font-size:13px}
  .video-stack{display:flex;flex-direction:column;gap:12px}
  .video-frame{width:100%;height:200px;border-radius:8px;border:0}
  .topright{display:flex;gap:8px;align-items:center}
  .search{width:360px}
  .lang-select, .currency-select{padding:6px;border-radius:6px;border:1px solid #e6eef6}
  .badge{background:rgba(255,255,255,0.12);padding:6px 8px;border-radius:8px}
  @media (max-width:1000px){.grid{grid-template-columns:repeat(2,1fr)}aside{width:300px}.search{width:200px}}
  @media (max-width:760px){.layout{flex-direction:column}aside{width:100%}.grid{grid-template-columns:1fr}.search{width:100%}}
  /* small helpers */
...
<header>
  <div class="brand">
    <div class="logo" id="logoText">AK</div>
    <div>
      <div style="font-weight:800" id="siteTitle">Anu Krupa Marketplace</div>
      <div class="small muted" id="siteTag">Verified sellers • Delivery Video Proof • Admin payouts</div>
    </div>
  </div>

  <nav>
    <a href="#home" onclick="scrollToId('home')">Home</a>
    <a href="#products" onclick="scrollToId('products')">Products</a>
    <a href="#about" onclick="scrollToId('about')">About</a>
    <div class="topright">
      <input id="search" class="search" placeholder="Search product, category or seller" oninput="onSearch()" />
      <select id="currency" class="currency-select" onchange="onCurrencyChange()"></select>
      <select id="lang" class="lang-select" onchange="setLang(this.value)"><option value="en">English</option><option value="hi">हिन्दी</option></select>
      <div id="navRight"></div>
    </div>
  </nav>
</header>
...
<div class="container">
  <section class="hero" id="home">
    <div>
      <h1 id="heroTitle">Anu Krupa — Trusted Marketplace</h1>
      <p class="lead" id="heroSub">Sellers can publish unlimited items. Admin controlled payouts and delivery video proof. Multi-language & multi-country demo (UPI simulated).</p>
      <div style="margin-top:12px;display:flex;gap:8px">
        <button class="btn btn-primary" onclick="showLogin()">Login / Register</button>
        <button class="btn btn-ghost" onclick="showSellerOnboard()">Become a Seller</button>
        <button class="btn btn-ghost" onclick="showAdminLogin()">Admin</button>
      </div>
    </div>
    <div style="width:360px" class="card">
      <div style="font-weight:700" id="settingsCardTitle">Quick Site Controls</div>
      <div class="small muted" style="margin-top:8px">Admin can change theme, commission, live videos and more.</div>
      <div style="margin-top:10px;display:flex;flex-direction:column;gap:8px">
        <button class="btn btn-primary" onclick="showSettings()">Site Settings</button>
        <button class="btn btn-ghost" onclick="showAdmin()">Open Admin Panel</button>
      </div>
    </div>
  </section>
